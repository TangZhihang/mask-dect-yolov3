...
def train(
    ...
    latest = models + 'latest/latest.pt'
    best = models + ' best/best.pt'
    ...
    plt.xlabel('LR')
    plt.tight_layout()
    plt.savefig('../result-graphs/LR.png', dpi=300)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Write epoch results
        with open('../result-graphs/results.txt', 'a') as file:
        ...
        if save:
            ...
            if epoch > 0 and epoch % 10 == 0:
                os.makedirs(models + os.sep+str(epoch),exist_ok=True)
                torch.save(chkpt, models + os.sep+str(epoch)+os.sep+'backup%g.pt' % epoch)
